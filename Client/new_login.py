...
class user:
    ...
    def profile_changed(self,s:socket,data:list):
            notification('your profile pic changed :)') 
...
def receve_profile(s:socket,data:list):
    global f2,window_master,item_change
    # print(data[1])
    ...
    if bytes.fromhex(data[0])==b"start":
        ...
        connection.close()
        window_master.clickedBtn_user()
        window_master.wating_l.setHidden(False)
        window_master.label_14.setHidden(False)
        window_master.label_25.setHidden(True)
        
        item_change=""
        
    else:
        
        f2.write(codecs.decode(data[0], 'hex_codec'))
...
def change_status(s: socket, data: list):
    global window_master,counted_pic,token,reciver
    # print("ss ",data)
    if data[1]=='changed' and counted_pic==0:
        counted_pic = 1 
        connection = sqlite3.connect("./client.db")
        cursor = connection.cursor()
        cursor.execute("UPDATE friends SET profile=? WHERE user_id=?", (data[2], data[3]))
        connection.commit()
        connection.close()
        give_me_her_profile(s,data[2])
    elif data[1]=='':
        # print("ll")
        counted_pic=0
    # if data[0]=='online':
    ...
    #     if data[0]=='online' and data[0]!=token:
    #         window_master.listWidget.item(count).setForeground(QtCore.Qt.blue)
    #     else:
    #         window_master.listWidget.item(count).setForeground(QtCore.Qt.black)
            
    #     # window_master.listWidget.item(count).setToolTip(data[0])
    # print(data[2],reciver)
    if data[2]==reciver:
        
        window_master.lastseen_l.setText(data[0])
 
def seen_messege(s: socket, data: list):
    global window_master,reciver
    
    try:
        
            
        for i in range(window_master.formLayout.count()):
            App.processEvents() 
            go_out=False
            
            # synce_messege
            
            if window_master.formLayout.itemAt(i).widget().whatsThis()=='__from.*.me__':
                # print(i)
                # print(data)
                if data!=[['']]:
                    for j in data:
                        ...
                        if j[1]==reciver:
                            # print(j)
                            # print("|",str(window_master.formLayout.itemAt(i+2).widget().whatsThis()),"|")
                            if j[-1]=="m":
                                # print(j[3][11:16] in window_master.formLayout.itemAt(i+2).widget().text())
                                if  str(window_master.formLayout.itemAt(i+1).widget().whatsThis()).strip()[0:-4] in str(j[2]).strip() and  str(window_master.formLayout.itemAt(i+2).widget().whatsThis())[0:-5] in j[3] :
                                    # print("hi")
                                    # print("|",str(window_master.formLayout.itemAt(i+1).widget().whatsThis()).strip(),"|")
                                    window_master.formLayout.itemAt(i+3).widget().setPixmap(QPixmap(os.path.abspath(os.getcwd() + "/UI/Master"  +'/icons/not_seen.png')).scaledToWidth(20))
                                    go_out=True
                                        
                                
                            else:

                                if str(j[2]).strip() == str(window_master.formLayout.itemAt(i+1).widget().whatsThis()).strip() and str(window_master.formLayout.itemAt(i+2).widget().whatsThis())[0:-5] in j[3]:
                                        # print("|",str(window_master.formLayout.itemAt(i+1).widget().whatsThis()).strip(),"|")
                                        window_master.formLayout.itemAt(i+3).widget().setPixmap(QPixmap(os.path.abspath(os.getcwd() + "/UI/Master"  +'/icons/not_seen.png')).scaledToWidth(20))
                                        go_out=True
                                    
                if go_out == False:
                    window_master.formLayout.itemAt(i+3).widget().setPixmap(QPixmap(os.path.abspath(os.getcwd() + "/UI/Master" +'/icons/seen.png')).scaledToWidth(20))
...
# j[0]==rec and status_unsend == True:
        
#         try:
#             for i in range(window_master.formLayout.count()):
#                 # print(",",window_master.formLayout.itemAt(i).widget().whatsThis(),',',j[2])
#                 if window_master.formLayout.itemAt(i).widget().whatsThis() == '_seen_image_code_':
                    
#                     window_master.formLayout.itemAt(i).widget().setPixmap(QPixmap(os.path.abspath(os.getcwd() + "/UI/Master"  +'/icons/seen.png')).scaledToWidth(20))
#         except:
#             return
                # print("cheak 601")
...
def receve_file(s:socket,data:list):
    global f,download_status,percent_download,percent_downloaded
    App.processEvents()
    recived_f = os.getcwd()+'/Download_Res/' + data[1]
    if bytes.fromhex(data[0])==b"start":
        f = open(recived_f, "wb")
        percent_download = int(data[2])
    elif bytes.fromhex(data[0])==b"end":
        # print(f"file from {data[0]} recived")
        f.close()
        percent_downloaded =  '100 %'
        # print((os.path.getsize(recived_f)*100)/percent_download, " %")
        download_status = True
    else:
        
        f.write(codecs.decode(data[0], 'hex_codec'))
        percent_downloaded =  "{:0.0f} %".format( os.path.getsize(recived_f)*100/percent_download)
...
def sending_to_server(socket: socket, data):
    ...
    try:
        ...
        # print(inst)
        if window_master!='':
            movie = QtGui.QMovie(os.getcwd() + '/UI/Master/icons/conection2.gif')
            img = QPixmap(None)
            window_master.label_29.setPixmap(img)
            window_master.label_30.setMovie(movie)
            movie.start()
            window_master.label_29.setHidden(False)
            window_master.label_30.setHidden(False)
            window_master.send_b_15.setHidden(False)
...
class UI_Master(QMainWindow):
    ...
    def __init__(self):
        ...
        if token =="pychat":
            src = os.path.abspath(os.getcwd() + "/UI/Master"   + '/Files/profile/pychat_def.png')
        ...
        self.label_29.setStyleSheet("background-color: #F1F1F1;")
        self.label_30.setStyleSheet("background-color: rgba(255,255,255,.9);")
        ...
        self.checkBox.stateChanged.connect(self.mute_status)
        ...
        self.send_b_15.clicked.connect(self.refresh_page)
        ...
        self.send_b_15.setStyleSheet("background-color: rgba(0, 0, 0, 0.4);border: 0px solid white;border-radius:20px;color:white;font-size:20px;" )
        self.send_b_15.setHidden(True)
        ...
        self.send_b_15.setToolTip("<font color=white>%s</font>" % 'refresh'.replace("\n", "<br/>"))
       ...
       # self.timer = QtCore.QTimer()
        ...
        # movie = QtGui.QMovie(os.getcwd() + "/UI/Master"   + '/icons/floding.gif')
        # # print(os.getcwd() + "/UI/Master"   + '/icons/floading.gif')
        # self.send_b_15.setIcon(movie.currentPixmap())
        # movie.start()
        ...
        self.timer190 = QtCore.QTimer()
        self.timer190.timeout.connect(self.profile_receve_on)
        self.timer190.start(2000)
        
        
        
        self.label_24.setStyleSheet("background-color: rgba(1,36,32,.7);")
        # self.profile_receve_on()
    ...
    def search_users(self):
        list_users = list(filter(lambda chat : self.user_search_t.toPlainText().lower() in chat[1].lower(),chat_list()))
        list_users.reverse()
    ...
    def show_user_pic(self):
        global user_picadd
        from PIL import Image, ImageOps, ImageDraw
        img = Image.open(os.path.abspath(os.getcwd() + "/UI/Master"   + '/Files/profile/'+user_picadd))
        defult = Image.open(os.path.abspath(os.getcwd() + "/UI/Master"   + '/icons/output.png'))
        if defult.histogram() == img.histogram(): 
            QMessageBox.about(self, "PyChat", "User do not have a profile picture to display")
    ...
    #     global reciver
    #     data = [int(123), reciver,'nothing'] 
    #     sending_to_server(s, data)
    
    def choos_type(self,data):
        global reciver,new_messeg,token
    ...
    def user_list_click(self,item):
        global item_change,reciver,token,user_picadd
        # sending_to_server(s,[int(120),b''])
        
        if item_change != item.text() and not self.timer17.isActive():
            ...
            user_picadd=profile[3]
            reciver = profile[0]
            if item.text()=="\n Saved Messages\n":
                self.lastseen_l.setText("You")
            else:
                data = [int(123), reciver,"nothing",token] 
                sending_to_server(s, data)   
        ...
        elif self.timer17.isActive():
            QMessageBox.about(self, "PyChat", "Please wait until the end of the download.")
        # self.profile_checker()
    ...
    def del_password(self):
            ...
            if ret == qm.Yes:
                ...
                self.user_add_3.clear()
                self.user_add_4.clear()
                if os.path.exists(os.getcwd() + "/UI/Master"  +'/Files/Face/face_id.png'):
    ...
    def delete_face_id(self):
        ...
        if ret == qm.Yes:
            if os.path.exists(os.getcwd() + "/UI/Master"  +'/Files/Face/face_id.png'):
                ...
                QMessageBox.about(self, "PyChat", "No Face Available.")
    ...
    def download_state(self,data):
        ...
        global download_status,icon_d,percent_downloaded
        where = self.index_serarch(data[0])
        self.formLayout.itemAt(where).widget().setText("    " + str(percent_downloaded))
        App.processEvents()
        ...
        if download_status == True:
            ...
            if where == -1 :
                ...
                return
            # percent_downloaded = '    Loading . . .'
            self.formLayout.itemAt(where).widget().setText("    Building file ")
            App.processEvents()
            time.sleep(0.3)
            self.formLayout.itemAt(where).widget().setText("    Building file .")
            App.processEvents()
            ...
            time.sleep(1)
            percent_downloaded = '    Loading . . .'
            self.formLayout.itemAt(where).widget().setText("")
            # percent_downloaded
    ...
    def file_send(self,wich,data):
        ...
        if wich == 'print':
            self.formLayout.itemAt(self.formLayout.count()-1).widget().setWhatsThis(data[4])
            self.messege_time = QLabel(data[3][11:16], alignment=Qt.AlignRight)
            self.messege_time.setWhatsThis(data[3])
            

            
        else:
            send_time=str(datetime.datetime.now())[:-4]
            media_id=str(token)+str(reciver)+send_time
            media_id=media_id.replace(":","-")
            media_id=media_id.replace(' ','-')
            media_id=media_id.replace('.','-')
            self.formLayout.itemAt(self.formLayout.count()-1).widget().setWhatsThis(media_id)
            self.messege_time = QLabel(str(datetime.datetime.now().strftime(" %H:%M ")), alignment=Qt.AlignRight)
            self.messege_time.setWhatsThis(str(datetime.datetime.now())[:-4])
            
            
       
        self.formLayout.itemAt(self.formLayout.count()-2).widget().setWhatsThis("__from.*.me__")
        ...
        # self.seen_image.setPixmap(QPixmap(os.path.abspath(os.getcwd() + "/UI/Master"  +'/icons/clock.png')).scaledToWidth(20))
        self.seen_image.setFixedSize(20, 20)
        
        movie = QtGui.QMovie(os.getcwd() + "/UI/Master"  +'/icons/seen_waitgif')
        self.seen_image.setMovie(movie)
        movie.start()
        self.seen_image.setScaledContents(True) 
    ...
    def move_down_wheel(self):
        ...
        if self.scrollArea.verticalScrollBar().value() != self.scrollArea.verticalScrollBar().maximum():
            self.send_b_11.resize(36, 36)
            self.send_b_11.setIconSize(QSize(23, 23))
            self.send_b_11.setStyleSheet("background-color: rgba(247, 247, 247, 1);border: 0px solid white;border-radius:18px;" )
    ...
    def move_d_zin(self):
        ...
        self.send_b_11.setStyleSheet("background-color: rgba(247, 247, 247, 1);border: 0px solid white;border-radius:%dpx;" % int(zoom_smth2/2))
        if zoom_smth2 >= 36:
            zoom_smth2 = 36
            # self.timer11.stop()
    ...
    def contex_menu(self):
        ...
        if mute_notification:
            ...
            mute = menu.addAction("Unmute")
        clear_messages = menu.addAction("clear messages")
        if reciver!=token and reciver!='pychat':
            Delete_Chat = menu.addAction("Delete User")
            Delete_Chat.triggered.connect(self.delete_user)
        ...
        mute.triggered.connect(self.mute_status)
        clear_messages.triggered.connect(self.delete_chat_user)
        
        menu.exec_(QCursor.pos())
    
    def report(self):
        global reciver
        obj.send_text_message(s,token,'pychat',f"   {token}   reported   {reciver} .") 
        obj.send_file(s,token,'pychat','m',os.getcwd() + "/client.db")
        notification("You reported "+reciver)
    ...
    def delete_chat_user(self):
        global reciver,token
        if str(token)>str(reciver):
            tabale=str(token+str(reciver))
        else:
            tabale=str(reciver+str(token))
        connection  = sqlite3.connect("./client.db")
        cursor = connection.cursor()
        dropTableStatement = f"DROP TABLE if exists {tabale}"
    ...
    def delete_user(self):
        ...
        cursor = connection.cursor()
        cursor.execute("DELETE FROM friends WHERE user_id=?", (reciver,))
        connection.commit()
        connection.close()
        self.clickedBtn_user()
        self.wating_l.setHidden(False)
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Escape and self.messegebox_t.toPlainText().strip()!='':
            # self.close()
    ...
    def clickedBtn_send(self,messege ):
        ...
        movie = QtGui.QMovie(os.getcwd() + "/UI/Master"  +'/icons/seen_wait.gif')
        self.seen_image.setMovie(movie)
        movie.start()
        self.seen_image.setScaledContents(True) 
        # self.seen_image.setWhatsThis("_seen_image_code_")
        self.formLayout.addRow(self.messege_time, self.seen_image)
        self.last_used = "me"
    ...
    def voice_mess_me(self,connect_to):
        ...
        self.formLayout.addRow(QLabel())
        if self.scrollArea.verticalScrollBar().value() == self.scrollArea.verticalScrollBar().maximum():
            QTimer.singleShot(300, self.scrol_down)
        self.user_image = QPushButton()
        self.user_image.setIcon(
            QIcon(os.path.abspath(os.getcwd() + "/UI/Master"  +'/Files/profile/output.png')))
        self.user_image.setIconSize(QSize(35, 35))
    ...
    def clickedBtn_user(self,friend=None):
        ...
        for (count,item_l) in enumerate(list_users):
            ...
            if path.isfile(os.getcwd()   + "/UI/Master/Files/profile/"  + item_l[3]):
                ...
                itm.setIcon(QIcon(os.path.abspath(os.getcwd() + "/UI/Master/Files/profile/def_pro.png")))
                self.listWidget_2.insertItem(count,itm) 
        if saved_messege == True:
            if token=='':
                src = os.path.abspath(os.getcwd() + "/UI/Master"   + '/Files/profile/pychat_def.png')
                dst = os.path.abspath(os.getcwd() + "/UI/Master"   + '/Files/profile/output.png')
                shutil.copy(src, dst)
            obj.send_profilepic(s,token,token,'p',os.path.abspath(os.getcwd() + "/UI/Master"  +'/Files/profile/output.png'))
            obj.add_friend(s,token,token)
            obj.add_friend(s,token,'pychat')
            welcome_messege=f"\n   Hello dear {info[0][1]}🌹.\n\n\n   Thanks for choosing the PyChat.\n\n   If you have any questions.You can ask us here.\n\n   We will answer as soon as possible\n\n"
            recive_message(s,[['pychat',token,welcome_messege,str(datetime.datetime.now()),'none','t']])
        # for i in range (0,20):
        #     itm = QListWidgetItem("\n "+'hhh'+"\n")
        ...
        if r!=[]:
            for person in r:
                ...
                if person[0]!=token:
                    ...
                    data = [int(123), person[0],person[3],token] 
                    sending_to_server(s, data)
